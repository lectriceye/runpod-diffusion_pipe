...
git clone --branch "$BRANCH" https://github.com/lectriceye/runpod-diffusion_pipe.git /tmp/runpod-diffusion_pipe

# Check if clone was successful
if [ $? -ne 0 ]; then
    echo "Error: Failed to clone branch '$BRANCH'. Falling back to main branch..."
    git clone https://github.com/lectriceye/runpod-diffusion_pipe.git /tmp/runpod-diffusion_pipe
